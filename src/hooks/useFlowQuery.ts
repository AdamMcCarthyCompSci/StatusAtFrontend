...
export const flowKeys = {
  ...
  tenant: (tenantUuid: string) =>
    [...flowKeys.all, 'tenant', tenantUuid] as const,
  lists: (tenantUuid: string, params?: FlowListParams) =>
    [...flowKeys.tenant(tenantUuid), 'list', params] as const,
  flow: (tenantUuid: string, flowUuid: string) =>
    [...flowKeys.tenant(tenantUuid), 'flow', flowUuid] as const,
};
  ...
  return useMutation({
    ...
    onSuccess: (newFlow, { tenantUuid }) => {
      ...
      queryClient.setQueryData(
        flowKeys.flow(tenantUuid, newFlow.uuid),
        newFlow
      );
    },
    onError: error => {
...
export function useUpdateFlow() {
  ...
  return useMutation({
    ...
    onSuccess: (updatedFlow, { tenantUuid, flowUuid }) => {
      ...
      queryClient.setQueryData(
        flowKeys.flow(tenantUuid, flowUuid),
        updatedFlow
      );
...
export function useDeleteFlow() {
  ...
  return useMutation({
    ...
    onSuccess: (_, { tenantUuid, flowUuid }) => {
      ...
      // Invalidate enrollment stats to refresh dashboard totals and recent updates
      queryClient.invalidateQueries({
        queryKey: ['enrollments', tenantUuid, 'stats'],
      });
      queryClient.invalidateQueries({ queryKey: ['enrollments'] });

      // Invalidate user data to refresh "Your Organizations" section
